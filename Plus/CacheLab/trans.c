...
void transpose_submit(int M, int N, int A[N][M], int B[M][N])
{
	...
	for(row_Block = 0;row_Block < N ;row_Block+=step){
		for(col_Block =0 ;col_Block < M; col_Block+=step){
			for(i=row_Block ; i<row_Block+step && i<N;i++){
				for(j=col_Block;j<col_Block+step &&j<M;j++){
					if(i!=j){
						B[j][i] = A[i][j];
					}else{
